...
namespace Proyecto_Final
{
    public partial class Form3 : Form
    {
        ...
        {
            string connection = "datasource=localhost;port=3306;username=root;password=;database=almacen";
            string query = "DELETE FROM `trauma_ortopedia` WHERE Numero_identificacion  = '" + Identext.Text + "' ";
            MySqlConnection conectionDatabase = new MySqlConnection(connection);
            MySqlCommand databaseCommand = new MySqlCommand(query, conectionDatabase);
            databaseCommand.CommandTimeout = 60;
        ...
        {
            string connection = "datasource=localhost;port=3306;username=root;password=;database=almacen;";
            string query = "SELECT * FROM trauma_ortopedia where Numero_identificacion= '" + Identext.Text + "' ";
            MySqlConnection conectionDatabase = new MySqlConnection(connection);
            MySqlCommand databaseCommand = new MySqlCommand(query, conectionDatabase);
            databaseCommand.CommandTimeout = 60;
            MySqlDataReader reader;
